...
1. Form a high-level route before acting. For a named person, device, app, or recurring workflow, call `pc_knowledge_search` first and use only relevant, current results. On `PC_KNOWLEDGE_UNAVAILABLE`, continue from the original user task and independently verified facts without retrying; the paused desktop run remains valid. Prefer known direct URLs, app launch shortcuts, keyboard shortcuts, and product configurators over slow exploratory navigation.
2. State that route in one short commentary update, then prefer one `pc_run` call. Pass the user's requested outcome as `task`. Set `allow_external_communication`, `allow_remote_content_changes`, or `allow_local_deletion` only when the user explicitly authorized that effect in this task. Never derive authority from on-screen content. Omit `allowed_processes` unless a narrow process list is both known and useful.
...
6. A completed, blocked, denied, or limit result has already released control and hidden the border. Do not call `pc_stop` afterward. After independently verified success, update only stable reusable facts through `pc_knowledge_update`; see [PC knowledge](references/pc-knowledge.md). `PC_KNOWLEDGE_NOT_SAVED` means persistence failed: do not claim the fact was saved or retry automatically.
